...
echo "=============================="
echo ""
echo "!! NOT covered by this script — role-specific NAT/DNAT rules !!"
echo "   This script only enables ip_forward. The ingress router ALSO needs"
echo "   (see network_architecture.docx section 3.4 and"
echo "   docs/infrastructure_routing.md section 11):"
echo "     iptables -t nat -A POSTROUTING -o eth0 -j MASQUERADE"
echo "     iptables -t nat -A PREROUTING -i enp7s0 -d 10.1.0.0/16 -j DNAT --to-destination <egress-router-public-ip>"
echo "   The egress router needs its own DNAT-to-webserver + MASQUERADE pair"
echo "   (same doc). These rules are RUNTIME-ONLY -- install iptables-persistent"
echo "   and run 'netfilter-persistent save' after adding them, or they silently"
echo "   vanish on the next reboot (this is exactly what caused the 2026-07-21"
echo "   nym5 regression documented in infrastructure_routing.md section 11 --"
echo "   the ingress router's MASQUERADE rule fell off on a reboot and nothing"
echo "   ever re-added it, since it isn't part of this bootstrap script)."